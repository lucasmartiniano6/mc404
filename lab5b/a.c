...
int main()
{
    ...
    */
    ...
    if(inst.type == R){
        ...
        ans |= inst.funct3 << 12;
        ans |= inst.rs1 << 15;
        ans |= (inst.imm & (fexp(2,12)-1)) << 20;         
    } else if (inst.type == S){
        ans |= inst.opcode;
        ans |= (inst.imm & (fexp(2,5)-1)) << 7;
        ans |= inst.funct3 << 12;
        ans |= inst.rs1 << 15;
        ans |= inst.rs2 << 20;
        ans |= ((inst.imm & 4064)>>5) << 25; // 4064 = (2**12 - 1) ^ (2**5 - 1)
    } else if (inst.type == B){
        ans |= inst.opcode;
        ans |= ((inst.imm & fexp(2,11))>>11) << 7;
        ans |= ((inst.imm & 30)>>1) << 8; // 30 = (2**5 - 1) ^ 1
        ans |= inst.funct3 << 12;
        ans |= inst.rs1 << 15;
        ans |= inst.rs2 << 20;
        ans |= ((inst.imm & 2016)>>5) << 25; // 2016 = (2**11 - 1) ^ (2**5 - 1) 
        ans |= ((inst.imm & fexp(2,12))>>12) << 31;
    } else if (inst.type == U){
        ans |= inst.opcode;
        ans |= inst.rd << 7; 
        ans |= (inst.imm & (fexp(2,21)-1)) << 12;
    } else if (inst.type == J){
        ans |= inst.opcode;
        ans |= inst.rd << 7; 
        ans |= ((inst.imm & 1044480)>>12) << 12; // 1044480 = (2**20 -1) ^ (2**12 -1)
        ans |= ((inst.imm & fexp(2,11))>>11) << 20; 
        ans |= ((inst.imm & 2046)>>1) << 21; // = (2**11 -1) ^ (1)
        ans |= ((inst.imm & fexp(2,20))>>20) << 31;
    }
